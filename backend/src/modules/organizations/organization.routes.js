...
organizationRouter.post('/', allowRoles(SUPER_ADMIN), async (req, res) => {
  ...
  // Best effort: don't fail org creation if analytics service is temporarily down.
  analyticsService
    .upsertOrganization({
    ...
    .then(async () => {
      ...
      logger.info({ organizationId }, 'Analytics org sync succeeded after admin org creation');
    })
    .catch(async (error) => {
      ...
      logger.warn(
        ...
        'Analytics org sync failed after admin org creation; daily reconciler will retry',
      );
    });
